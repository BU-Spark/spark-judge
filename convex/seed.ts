  ...
  handler: async (ctx) => {
    ...
    for (const courseCode of courseCodes) {
      ...
      for (let i = 0; i < projectsToCreate; i++) {
        ...
        const teamName =
          teamNameTemplates[i % teamNameTemplates.length] +
          ` ${courseCode} ${i + 1}`;
        const description =
        ...
        await ctx.db.insert("teams", {
          ...
          projectUrl: `https://github.com/${courseCode.toLowerCase().replace(
            "/",
            "-"
          )}/project-${i + 1}`,
          githubUrl: `https://github.com/${courseCode.toLowerCase().replace(
            "/",
            "-"
          )}/project-${i + 1}`,
          track: "",
...
export const seedCohortJudgingDemo = mutation({
  ...
  handler: async (ctx) => {
    ...
    const currentUserJudgeId = await ctx.db.insert("judges", {
      userId,
      eventId,
    });
    ...
    const teamNamePrefixes = [
      "Quantum", "Neural", "Cloud", "Data", "AI", "Blockchain", "Mobile", "Web",
      "VR", "AR", "IoT", "Cybersecurity", "FinTech", "HealthTech", "EdTech", "Green",
      "Social", "Gaming", "Music", "Art", "Food", "Travel", "Fitness", "Finance",
      "Education", "Healthcare", "Retail", "Logistics", "Real Estate", "Media",
    ];

    const teamNameSuffixes = [
      "Labs", "Studio", "Works", "Solutions", "Systems", "Platform", "Hub", "Space",
      "Forge", "Builders", "Creators", "Innovators", "Pioneers", "Ventures", "Tech",
      "Digital", "Smart", "Next", "Future", "Prime", "Elite", "Pro", "Plus",
    ];
    ...
    const memberNames = [
      ...
      "Dakota", "River", "Phoenix", "Blake", "Cameron", "Avery", "Finley",
      "Rowan", "Skylar", "Emery", "Hayden", "Reese", "Parker", "Drew", "Logan",
      "Sam", "Jamie", "Charlie", "Noah", "Emma", "Olivia", "Liam", "Sophia",
    ...
    for (let i = 0; i < 50; i++) {
      ...
      const suffix = teamNameSuffixes[Math.floor(i / teamNamePrefixes.length) % teamNameSuffixes.length];
      const teamName = `${prefix} ${suffix}${i >= teamNamePrefixes.length ? ` ${Math.floor(i / (teamNamePrefixes.length * teamNameSuffixes.length)) + 1}` : ""}`;
    ...
    for (let judgeIdx = 0; judgeIdx < judgeIds.length; judgeIdx++) {
      ...
      for (let i = 0; i < teamsToScore; i++) {
        ...
        const categoryScores = event.categories.map((catObj) => {
          ...
          const shouldOptOut =
            catObj.optOutAllowed && Math.random() < 0.1;
    ...
    for (let judgeIdx = 0; judgeIdx < judgeIds.length; judgeIdx++) {
      ...
      for (let i = 0; i < teamsToScore; i++) {
        ...
        const categoryScores = event.categories.map((catObj) => {
          ...
          const shouldOptOut =
            catObj.optOutAllowed && Math.random() < 0.05;
